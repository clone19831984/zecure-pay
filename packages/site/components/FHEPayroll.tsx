...
export const FHEPayroll = () => {
  ...
  const [amount, setAmount] = useState(""); // mặc định trống
  const [recipient, setRecipient] = useState("");
  const [contractDepositAmount, setContractDepositAmount] = useState(""); // mặc định trống
  const [withdrawAmount, setWithdrawAmount] = useState(""); // mặc định trống
  ...
  const handleGroupPayroll = () => {
    ...
    const amounts = selectedUsers.map(user => userAmounts[user] || "0");
    const validUsers = selectedUsers.filter((user, index) => amounts[index] && amounts[index] !== "0");
    ...
    return (
      <div className="flex flex-col items-center justify-center p-8">
        <div className="text-center">
          <p className="text-lg">Loading FHE Payroll System...</p>
        </div>
      </div>
    );
  }
    ...
    <>
      ...
      {isOwner && (
        <div className="mb-8 w-full max-w-4xl border border-gray-300 rounded-lg p-6 bg-white shadow-md">
          <h2 className="text-2xl font-bold mb-4 text-center">Fund Contract</h2>
          
          {/* Top Row: All 3 sections in one row */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Owner: Contract Balance */}
            <div>
              <button className="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-3 rounded w-full text-sm mb-3">
                Contract Balance
              </button>
              <div className="border p-2 rounded w-full mb-3 text-sm bg-white h-10 flex items-center">
                <p className="text-base font-bold">
                  <span className="text-blue-600">{contractBalance} USDT</span>
                </p>
              </div>
              <button
              ...
              >
                {status === "loading" ? "Loading..." : "Refresh Balance"}
              </button>
              ...
              {status === "loading" && (
                <button
                  ...
                  className="mt-2 bg-yellow-500 hover:bg-yellow-700 text-white text-xs py-1 px-2 rounded"
                >
                  Reset if Stuck
                </button>
              )}
            ...
            <div>
              ...
              <input
                ...
                value={contractDepositAmount}
                onChange={(e) => setContractDepositAmount(e.target.value)}
              ...
              >
                {status === "loading" ? "Processing..." : "Deposit to Contract"}
              </button>
            </div>
            ...
            <div>
              ...
              <button
                ...
                className="zama-yellow text-black font-bold py-2 px-3 rounded w-full text-sm"
              >
                {status === "loading" ? "Processing..." : "Withdraw to Owner"}
              </button>
            </div>
          </div>
        </div>
                      ...
                      {selectedUsers.map((user, index) => (
                        <div key={user} className="flex items-center gap-2 p-2 bg-gray-50 rounded">
                          ...
                          <input
                            ...
                            value={userAmounts[user] || ""}
                            onChange={(e) =>
                              setUserAmounts({ ...userAmounts, [user]: e.target.value })
                            }
      ...
      {/* User: Balance - chỉ hiện khi không phải owner */}
      {!isOwner && (
      ...
      {/* User: Withdraw - chỉ hiện khi không phải owner */}
      {!isOwner && (
        <div className="mb-8 w-full max-w-md">
          <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
